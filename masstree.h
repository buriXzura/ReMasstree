...
public:
    ...
    uint64_t tryInsertLock() {
        return __sync_fetch_and_or(&v,INSERT_LOCK);
    }
    uint64_t trySMOLock() {
        return __sync_fetch_and_or(&v,BOTH_LOCKS);
    }
    bool smoLock() {
        return v&SMO_LOCK;
    }
...
class inner_node
{
    private:
        inner_node              *parent,                            //8B
                                *right,                             //8B
                                *left;                              //8B
        std::atomic<uint64_t>   typeVersionLockObsolete{0b100};     //8B
        u_int64_t               highest;                            //8B
    ...
    public:
        ...
        inner_node(void *parent, void *right, void *left, u_int32_t level):
        ...
        kv split(u_int64_t key, void* value);
        ...
        static int capacity(){return LEAF_WIDTH+1;}
...
class leaf_node
{
    ...
    public:
        ...
        leaf_node(void *parent, void *right, void *left, u_int32_t level):
            parent(reinterpret_cast<inner_node *>(parent)),
        ...
        static int capacity(){return LEAF_WIDTH;}
...
class btree
{
    ...
    public:
        ...
        int height(){return level(root_)+1;}
